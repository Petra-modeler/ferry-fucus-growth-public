...
# Miksi alla oleva poistettiin, oliko se kuva jota Petra A:n puhelimelta katsottiin ja oli mennyt rikki
...
#####
m_biomass_2 <- glmmTMB(Tip.dry.weight.July..g. ~ Tip.dry.weight.June.ww_dw_ratio..g. + I.or.C + Genotype.origin + I.or.C:Genotype.origin + (1|SITE.ID) + (1|Tile.number) + (I.or.C|Genotype), data=df, family=gaussian)
summary(m_biomass_2)
pearson_resid_2 <- residuals(m_biomass_2, type = "pearson")
plot(pearson_resid_2)
# What data model has used
...
# Combine residuals with the data model used
model_data$id <- 1:nrow(model_data)   # yksilöivä tunniste
model_data$pearson_resid <- residuals(m, type = "pearson")

# Plot residuals with codes
plot(model_data$pearson_resid, pch = 19)
text(x = 1:nrow(model_data),
     y = model_data$pearson_resid,
     labels = model_data$id,
     pos = 3, cex = 0.7)

# Filter residuals under -0.4 to see pictures what is the problem with them
# Can we remove them from data, if they are for example eaten or broken? 
resid_under_neg04 <- subset(model_data, pearson_resid < -0.4)
